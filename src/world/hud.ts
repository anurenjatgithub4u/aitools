...
export interface Hud {
  ...
  table(power: number, pos: number | null, dir?: number): void;
  tableStatus(text: string): void;
  tableScore(a: { name: string; score: string; sub: string } | null, b: { name: string; score: string; sub: string } | null): void;
  shotClock(text: string | null): void;
  bowl(s: { pace: number; line: number } | null): void;
  shot(s: { shot: number; name: string; hint: string } | null): void;
  wanted(text: string | null): void;
...
export function renderHud(root: HTMLElement, d: Destination, points: number, actions: HudActions): Hud {
  ...
  <div class="hud">
    ...
    <div class="top-center">
      <div class="chip big" id="coinchip" title="Coins: pick them up around the city, spend them on rides">🏆 <b id="pts">${points}</b> points <span class="coins">🪙 <b id="coins">${store.coins()}</b></span></div>
      <div class="chip online" id="online"><i class="dot"></i> connecting…</div>
      <div class="row"><div class="chip rank" id="rank">🏅 Rank #–</div><div class="chip friends" id="friends">🤝 ${store.friends().length} friends</div><div class="chip zombie" id="zombiebtn" title="Zombie night comes on its own — be ready">🧟 Zombies in --:--</div><div class="chip wanted" id="wantedchip" title="The police are after you" hidden></div></div>
    </div>
    <div class="top-right">
    ...
    <div class="shotctl" id="shotctl" hidden>
      <div class="shotrow" id="shotrow">
        <button type="button" data-i="0" title="Block — dead bat, straight down the pitch">🛡</button>
        <button type="button" data-i="1" title="Drive — straight past the bowler">⬆</button>
        <button type="button" data-i="2" title="Cover — through the off side">↖</button>
        <button type="button" data-i="3" title="Cut — square on the off side">⬅</button>
        <button type="button" data-i="4" title="Flick — through mid-wicket">↗</button>
        <button type="button" data-i="5" title="Pull — square leg, up in the air">➡</button>
        <button type="button" data-i="6" title="Sweep — behind square, to fine leg">↘</button>
        <button type="button" data-i="7" title="Slog — over the top: six or caught">🚀</button>
      </div>
      <small id="shotname">Drive</small>
    </div>
    <div class="tablectl" id="tablectl" hidden>
      <div class="tboard" id="tboard">
        <div class="tbside" id="tbyou"><small>You</small><b>—</b><span></span></div>
        <div class="tbclock" id="tclock" hidden></div>
        <div class="tbside" id="tbopp"><small>Rival</small><b>—</b><span></span></div>
      </div>
      <div class="tstatus" id="tstatus"></div>
      <div class="trow"><span>Aim</span><button type="button" class="tbtn" id="aiml" title="Aim left (A)">◀</button><i class="taimdir" id="taimdir">➤</i><button type="button" class="tbtn" id="aimr" title="Aim right (D)">▶</button><small>hold · Shift = fine</small></div>
      ...
      <div class="trow presets" id="pwrpresets"><button type="button" data-p="30">Soft</button><button type="button" data-p="60">Medium</button><button type="button" data-p="100">Full</button></div>
      <div class="trow btns"><button type="button" class="act" id="cambtn" title="Change camera view">🎥<small>View</small></button><button type="button" class="act kick" id="strikebtn">🎱<small>Shoot</small></button><button type="button" class="act exit" id="texit" title="Leave the game (Esc)">✕<small>Exit</small></button></div>
    </div>
    ...
    <div class="help" id="helpbox" hidden>
      <h3>How to play</h3>
      <p><b>Move</b> W A S D or arrow keys · hold <b>Shift</b> to run (or tap the Run button to stay running) · <b>Space</b> to jump.<br><b>Look</b> drag with the mouse, or two-finger swipe on a touchpad · mouse wheel or pinch to zoom.<br><b>Drive</b> walk up to a jeep, tuk-tuk, bike or cycle and press <b>E</b> · W/S accelerate · A/D steer · Space brake · <b>Shift</b> nitro boost · <b>H</b> horn · E to get out.<br><b>People</b> walk up to any explorer and a card appears: send a <b>friend</b> request (G), start a <b>game</b> — race, football, prize hunt, 8-ball, carrom, chess or Ludo — <b>hang out</b> (they walk with you for a while) or <b>chat</b> (C opens the chat box; people nearby answer). Friends keep a 🤝 badge every time you visit.<br><b>Tasks</b> timed challenges appear in the top-left card (or press <b>T</b>): find hidden cash, dash through checkpoints, run a taxi job or gather snacks. Finish fast for up to double points.<br><b>Petrol</b> a tank lasts about 5 km (boosting burns double). When it runs dry, coast to the ⛽ station, stop, and press <b>R</b> to fill up. <b>M</b> toggles sound.<br><b>Touch</b> left half = joystick · right half = look · buttons for jump and drive.<br><b>Football</b> walk onto the City Stadium pitch and press <b>E</b> (or pick Football from an explorer's card): five-a-side, 90 seconds. Run into the ball to dribble (it sticks to your feet), <b>Space</b> / the Jump button to shoot — harder while running, and aimed toward the goal when you face it. Kick-offs are contested — only the restart team stands next to the ball, so a goal off the whistle has to be earned — and a defender who gets tight to you can win the ball back.<br><b>Cricket</b> walk onto the strip at the FindurAI Cricket Ground (Eastside) and press <b>E</b>, or pick Cricket on an explorer's card: pick 1, 2, 3 or 5 overs a side (2, 3 or 5 wickets), then call the <b>toss</b> — win it and you choose whether to bat or to bowl first. <b>Batting</b>: on a computer, pick where you mean to hit it on the small panel (or keys <b>1</b>–<b>8</b>) — block, drive, cover, cut, flick, pull, sweep or slog — then press <b>Bat</b> / <b>Space</b> as the ball reaches you. Perfect timing sends it exactly there; the bigger the shot, the tighter the timing, so a mistimed pull or slog balloons up for the fielders instead of clearing the rope. On a phone it is pure timing: perfect drives it for six, early pulls it high where the ring of fielders can catch it — or put it down, they are not perfect — late nicks it along the ground, and a straight one you miss bowls you. ◀ ▶ shuffle across the crease for a wide one. After a shot, press <b>Run</b> / <b>W</b> to run between the wickets with your partner — press again for another, but be home before the fielder's throw or you are run out. Fielders now gather every ball and throw it back into play themselves — to the stumps at your end while you bat, or safely to the bowler when you are the one fielding. <b>Bowling</b>: before each ball pick the <b>speed</b> and <b>line</b> on the panel, tap <b>Bowl</b> to run in, and press <b>Bowl</b> / <b>Space</b> again at the top of your action (bar in the green) for a good ball; loose balls get punished. Wickets +25, dots +5, win the match +200.<br><b>Neon Palace</b> the casino &amp; club at the end of Neon Lane: walk onto the dance floor and press <b>E</b> to dance (with whoever is with you), or press <b>E</b> at a pool table or the carrom board by the bar for a real game. Either way you are first asked for an optional <b>shot clock</b> — no timer, or 15 / 30 seconds to play each turn before it plays whatever you had lined up. A small scoreboard shows both sides' tallies from the first shot. Pool: ◀ ▶ / A D aim (Shift for fine aim), the Power slider or W/S sets the strength (or tap Soft / Medium / Full), <b>Shoot</b> strikes. Carrom: the Striker slider or Q/E slides the striker along your baseline, ◀ ▶ aim, Power slider or W/S, <b>Flick</b>. <b>View</b> cycles a few fixed camera angles on either table. <b>Exit</b> (or Esc) leaves any game. Lasers, DJ, bar, slots.<br><b>Dates</b> press <b>E</b> with someone at the coffee table at Chai Corner, the pier bench, the candle-lit table on the sand, the campfire, the sunset bench at the lighthouse, the Sunset Wheel or the marina boat. <b>Gift</b> on any card sends a rose, ice cream, chai, teddy, chocolate or a note.<br><b>Zombie night</b> comes on its own — the 🧟 chip counts down to it. Waves of zombies shamble toward you — walkers, headless ones, runners, crawlers, hoppers that leap at you, bloaters that burst in a cloud, and brutes that take a beating and hit like a truck. <b>Space</b> / Jump punches the one in front of you (three hits each), vehicles crush them. Every bite drains your ❤ health — clear a wave to heal, die and you wake up back downtown.<br><b>Rides</b> walk to the gate of any fairground ride and press <b>E</b> (5 🪙, and whoever is with you climbs in too): the Sky Wheel by Neon Lane, the Carousel in Central Park, the Chair Swing at the Skate Park, the Pirate Ship on the beach, the <b>Sky Drop</b> tower that hauls you up and lets go, the <b>Spinning Teacups</b> on the sand, the <b>Seaside Coaster</b> — a full circuit with a chain lift, a first drop and two camelbacks — the <b>Bumper Cars</b> by the Skate Park, the <b>Sky Flyer</b> that swings you out over the sea, and the <b>Spooky Express</b>, a little ghost train through the graveyard behind the fairground.<br><b>Police</b> a 🚔 patrol jeep works the city with an officer at the wheel. Knock someone down with a car, ram the jeep or fly past it at speed and you pick up ★ heat: the siren starts and it comes after you — on foot, on a bike, or in the fastest car in the city, it matches your pace and pushes harder the longer it stays behind, so outrunning it takes real distance, not just a burst of speed. Let it reach you while you are slow or on foot and you are fined <b>15 🪙 a star</b> — and if you have not got the coins, you do the time instead: 20–45 s in the cell at the City Police Station, −30 points. Get clear — well clear, and for a while — and it gives up and goes back to its beat.<br><b>Lifts</b> while driving slowly next to an explorer press <b>F</b> to pick them up, F again to drop them off for +30.<br><b>Collect</b> walk (or drive) over any floating item with a number. Gold 🪙 coins are scattered everywhere too — rides cost 5, and finishing a task pays 5.</p>
      <p>${d.blurb}</p>
  ...
  // quick power presets — Soft / Medium / Full, right next to the slider they set
  const pwrPresets = root.querySelector<HTMLElement>('#pwrpresets')!;
  pwrPresets.addEventListener('pointerdown', (e) => { const b = (e.target as HTMLElement).closest('button'); if (!b) return; e.preventDefault(); const v = Number(b.dataset.p); pwrRng.value = String(v); pwrVal.textContent = `${v}%`; actions.setPower(v / 100); });
  // cycle the fixed camera views on the table games
  press(root.querySelector<HTMLButtonElement>('#cambtn')!, actions.cycleCam);
  ...
  // shot picker (desktop only — hidden under 720px by CSS, but wired regardless in case a phone is later widened into that range)
  const shotEl = root.querySelector<HTMLElement>('#shotctl')!, shotRow = root.querySelector<HTMLElement>('#shotrow')!, shotName = root.querySelector<HTMLElement>('#shotname')!;
  shotRow.addEventListener('pointerdown', (e) => { const b = (e.target as HTMLElement).closest('button'); if (!b) return; e.preventDefault(); actions.setShot(Number(b.dataset.i)); });
  let hurtT = 0;
  ...
  return {
    coins(n) { const el = root.querySelector<HTMLElement>('#coins')!, chip = root.querySelector<HTMLElement>('#coinchip')!; el.textContent = String(n); chip.classList.remove('pop'); void chip.offsetWidth; chip.classList.add('pop'); },
    ...
    shot(s) { shotEl.hidden = !s; if (s) { segSel(shotRow, s.shot); shotName.textContent = s.name; shotName.title = s.hint; } },
    wanted(text) { const el = root.querySelector<HTMLElement>('#wantedchip')!; el.hidden = !text; if (text) el.textContent = text; },
    ...
    table(power, pos, dir) { if (!sliding) { pwrRng.value = String(Math.round(power * 100)); pwrVal.textContent = `${pwrRng.value}%`; if (pos !== null) posRng.value = String(Math.round(pos * 100)); } if (dir !== undefined) aimDir.style.transform = `rotate(${dir}rad)`; },
    tableScore(a, b) {
      const side = (id: string, s: { name: string; score: string; sub: string } | null) => { const el = root.querySelector<HTMLElement>(id)!; el.querySelector('small')!.textContent = s ? s.name : ''; el.querySelector('b')!.textContent = s ? s.score : '—'; el.querySelector('span')!.textContent = s ? s.sub : ''; };
      side('#tbyou', a); side('#tbopp', b);
    },
    shotClock(text) { const el = root.querySelector<HTMLElement>('#tclock')!; el.hidden = !text; if (text) el.textContent = text; },
    pick(p, choose) {
