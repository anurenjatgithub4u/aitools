...
export function renderHud(root: HTMLElement, d: Destination, points: number, actions: HudActions): Hud {
  ...
  <div class="hud">
    ...
    <div class="bowlctl" id="shotctl" hidden>
      <div class="brow"><span>Ground</span><div class="seg" id="segshota"><button type="button" data-i="0">🛡 Block</button><button type="button" data-i="1">⬆ Drive</button><button type="button" data-i="2">↖ Cover</button><button type="button" data-i="3">⬅ Cut</button></div></div>
      <div class="brow"><span>Attack</span><div class="seg" id="segshotb"><button type="button" data-i="4">↗ Flick</button><button type="button" data-i="5">➡ Pull</button><button type="button" data-i="6">↘ Sweep</button><button type="button" data-i="7">🚀 Slog</button></div></div>
      <small id="shothint">Pick where you mean to hit it · 1‑8 on the keyboard</small>
    </div>
  ...
  // the shot picker, while you bat
  const shotEl = root.querySelector<HTMLElement>('#shotctl')!, shotHint = root.querySelector<HTMLElement>('#shothint')!;
  const shotBtns = Array.from(shotEl.querySelectorAll('button'));
  shotEl.addEventListener('pointerdown', (e) => { const b = (e.target as HTMLElement).closest('button'); if (!b) return; e.preventDefault(); actions.setShot(Number(b.dataset.i)); });
  let hurtT = 0;
  ...
  return {
    ...
    shot(s) { shotEl.hidden = !s; if (s) { for (const b of shotBtns) b.classList.toggle('on', Number(b.dataset.i) === s.shot); shotHint.textContent = `${s.name} — ${s.hint}`; } },
